...
class CocoDataset_w_panoptic(CustomDataset):
    ...
    def evaluate(self,
        ...
        if 'panoptic' in metrics:
            ...
            print_log('Attention, this is a naive implement. The 2 channels of PNG(constrined by mmcv, it may be RBG format) should be saved in "./panoptic_seg file folder"', logger=logger)
            print_log('The eval pipeline has 2 steps: 1.convert 2channels format PNG to panoptic format. 2 eval with panopticapi', logger=logger)
            
            converter(
                self.cat_id_file_folder,
                self.img_info_file_folder,
                self.panoptic_coco_categories,
                self.output_folder,
                self.pred_json,
              )
