...
class Attention(nn.Module):
    ...
    def forward(self, query, key,value, key_padding_mask):
        ...
        if key_padding_mask is not None:
            ...
            #attn.masked_fill_(key_padding_mask, float(-1))
            
            attn.masked_fill_(key_padding_mask, float("-inf"))
            #attn2.masked_fill_(key_padding_mask, float(0))
            del key_padding_mask
        
         # B,NH, N,L
        mask = self.linear(attn2.permute(0,2,3,1))
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class Block(nn.Module):
    ...
    def forward(self, query, key,value, key_padding_mask = None,H=None,W=None):
 
        x,mask = self.attn(query,key, value, key_padding_mask)
        query = query + self.drop_path(x)
        query = self.head_norm1(query)
     
        query = query + self.drop_path(self.mlp(query,H,W))
...
class TransformerHead8(nn.Module):
    ...
    def forward(self, memory, mask_memory,pos_memory, query_embed, mask_query,pos_query,H,W):
        if mask_memory is not None and isinstance(mask_memory,torch.Tensor):
            mask_memory = mask_memory.to(torch.bool)
        masks = []
        for block in self.blocks:
            query_embed,mask = block(self.with_pos_embed(query_embed, pos_query),self.with_pos_embed(memory, pos_memory),memory, key_padding_mask= mask_memory,H=H,W=W)
            masks.append(mask)
