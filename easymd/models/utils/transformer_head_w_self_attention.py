...
class AttentionTail(nn.Module):
    def __init__(self, cfg,dim, num_heads=2, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        self.q = nn.Linear(dim, dim, bias=qkv_bias)
        self.k = nn.Linear(dim, dim, bias=qkv_bias)
        self.self_attention = SelfAttention(cfg,dim,num_heads=num_heads,qkv_bias=qkv_bias,qk_scale=qk_scale,attn_drop=attn_drop,proj_drop=proj_drop)
...
class Block(nn.Module):
    ...
    def forward(self, query, key,value, key_padding_mask = None):
        ...
        query = query + self.drop_path(self.attn(query,key, value, key_padding_mask))
...
class TransformerHead_w_st(nn.Module):
    ...
    def forward(self, memory, mask_memory,pos_memory, query_embed, mask_query,pos_query):
        ...
        for block in self.blocks:
            query_embed = block(self.with_pos_embed(query_embed, pos_query),self.with_pos_embed(memory, pos_memory),memory, key_padding_mask= mask_memory)
