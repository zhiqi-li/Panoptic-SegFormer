...
class Attention(nn.Module):
    ...
    def forward(self, query, key,value, key_padding_mask,hw_lvl):
        ...
        #m = feats_l1.permute(0,1,3,2)
        #am = am.reshape(-1,wedge_1)
        #am_max = am.max(-1)[0][...,None]
        #am_min = am.min(-1)[0][...,None]
        #am = (am-am_min)/(am_max-am_min)
        #am = am.reshape(-1,*hw_lvl[0]) 
        #print(am.shape)
        #global count
        #maps = {0:31,1:31,2:29,3:24}
        #save_tensor(am[maps[count]:maps[count]+1],'ptt_10_cat_test_{i}.png'.format(i=count),convert=True)
        #count+=1
        ###
        ...
        feats_l2 = self.linear_l2(feats_l2).permute(0,1,3,2).reshape(-1,self.num_heads,*hw_lvl[1])  # B,NH, N,L
        feats_l3 = self.linear_l3(feats_l3).permute(0,1,3,2).reshape(-1,self.num_heads,*hw_lvl[2]) 
        #print(feats_l2.shape)
        feats_l2 = F.interpolate(feats_l2,size=hw_lvl[0],mode='bilinear').permute(0,2,3,1).reshape(B,N,wedge_1,self.num_heads)
        feats_l3 = F.interpolate(feats_l3,size=hw_lvl[0],mode='bilinear').permute(0,2,3,1).reshape(B,N,wedge_1,self.num_heads)
...
class AttentionTail(nn.Module):
    ...
    def forward(self, query, key, key_padding_mask,hw_lvl=None):
        ...
        am = am.reshape(-1,*hw_lvl[0]) 
        print(am.shape)
        save_tensor(am,'ptt_9.png',convert=True)'''
...
class TransformerHead12(nn.Module):
    ...
    @force_fp32(apply_to=('memory', 'mask_memory','pos_memory','query_embed','mask_query','pos_query'))
