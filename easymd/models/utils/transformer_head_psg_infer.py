...
class Attention(nn.Module):
    ...
    def forward(self, query, key,value, key_padding_mask,hw_lvl):
        ...
        #print('attn',attn.shape,hw_lvl)
            ...
            feats_l2 = F.interpolate(feats_l2,size=hw_lvl[0],mode='nearest').permute(0,2,3,1).reshape(B,N,wedge_1,8)
            feats_l3 = F.interpolate(feats_l3,size=hw_lvl[0],mode='nearest').permute(0,2,3,1).reshape(B,N,wedge_1,8)
...
class AttentionTail(nn.Module):
    ...
    def forward(self, query, key, key_padding_mask,hw_lvl=None):
        ...
        #save_tensor(img_[16:20],'img_{i}_2.png'.format(i=self.count)) 
...
class TransformerHead_psg_infer(nn.Module):
