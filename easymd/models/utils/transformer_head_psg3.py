...
class Attention(nn.Module):
    ...
    def forward(self, query, key,value, key_padding_mask,hw_lvl):
        ...
        if self.training:
            ...
            #img_ = feats_l1.softmax(-1).permute(0,1,3,2).reshape(-1,*hw_lvl[0])
            #save_tensor(img_,'img_{i}_1.png'.format(i=self.count))
            feats_l2 = attn_mask[:,:,wedge_1:wedge_2,:].permute(0,1,3,2).reshape(-1,8,*hw_lvl[1]) 
            feats_l3 = attn_mask[:,:,wedge_2:,:].permute(0,1,3,2).reshape(-1,8,*hw_lvl[2]) 
            ...
            feats_l2 = self.linear_l2(feats_l2) # B,NH, N,L
            feats_l3 = self.linear_l3(feats_l3)
            #print(feats_l2.shape)
        
            #print(feats_l1.shape,feats_l2.shape,feats_l3.shape)
            new_feats = torch.cat([feats_l1,feats_l2,feats_l3],-1)
            new_feats = new_feats.permute(0,1,3,2).reshape(B*N, 24,*hw_lvl[0])
            new_feats = F.interpolate(new_feats,scale_factor=2,mode='bilinear')
            new_feats =new_feats.permute(0,2,3,1)
            s1,s2,s3,s4 = new_feats.shape
            new_feats = new_feats.reshape(B,N,s2*s3,24)
...
class TransformerHead_psg3(nn.Module):
